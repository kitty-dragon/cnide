...
class Segmenter_ {
  ...
  parseNetwork(cn, colors) {
    ...
    for (let i = 0; i < this.positions_.length; i++) {
      ...
      combinator.xPos =
          Math.floor(i / ROW_SIZE) % 2 == 0 ?
          i % ROW_SIZE :
          ROW_SIZE - (i % ROW_SIZE) - 1;
      combinator.yPos = Math.floor(i / ROW_SIZE) * 2;
      let needPoles = { input: false, output: false };
      for (const conn of getConnections_(combinator)) {
        const nextConnection = wires[conn.wire][1];
        if (!nextConnection) continue;
        let nextCombinatorId = this.positions_.indexOf(nextConnection.combinator);
      ...
      for (let k of [ 'input', 'output' ]) {
        ...
        const poleIndex = i + MAX_DISTANCE;
        ...
        for (const conn of getConnections_(combinator)) {
          if (conn.hOffset !== (k === 'input' ? 0 : 1)) continue;
