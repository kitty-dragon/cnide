...
CnideProgram
  = _ statements:(CnideTopLevelStatement _)* !. {
      return statements.map(st => st[0]);
    }

CnideTopLevelStatement
...
Assertion
  = operator:DeciderOperator _ value:Integer {
    return [ operator, value ]
  }

AssertKeyValue
  = key:Signal _ value:Assertion {
      return { key: key.name, operator: value[0], value: value[1] };
    }
  / key:Each _ value:Assertion {
      return { key, operator: value[0], value: value[1] };
    }

AssertKeyValues
  = kv:AssertKeyValue _ "," _ rest:AssertKeyValues {
      rest.push(kv);
      return rest;
    }
  / kv:AssertKeyValue {
...
PulseTestStatement
  = range:Range _ "pulse" _ wire:Wire _ "{" _ values:KeyValues _ "}" _ {
      return { type: 'pulse', start: range[0], end: range[1], wire, values }
    }

AssertTestStatement
  = range:Range _ "assert" _ wire:Wire _ "{" _ values:AssertKeyValues _ "}" _ {
      return { type: 'assert', start: range[0], end: range[1], wire, values }
    }

PrintTestStatement
  = range:Range _ "print" _ wire:Wire _ {
      return { type: 'print', start: range[0], end: range[1], wire }
    }
...
TestCase
  = "test" _ netname:NetworkName _
    "{" _ nodes:(TestStatement _)* _ "}" {
